        ...
        private void OnTriggerEnter2D(Collider2D other)
        {
            switch (area)
            {
                case Area.red:
                    ...
                    AudioManager.Play(AudioManager.SearchForTrack("Guitar"), loop: true, time: AudioManager.SearchForSong("Main").GetTrackTime());
                    break;
                case Area.green:
                    AudioManager.FadeOut(AudioManager.SearchForTrack("Flute"), AudioManager.defaultCurve);
                    AudioManager.FadeIn(AudioManager.SearchForTrack("Percussion"), AudioManager.defaultCurve, loop: true, fadeDuration: 3, AudioManager.SearchForSong("Main").GetTrackTime());
                    AudioManager.FadeIn(AudioManager.SearchForTrack("Bass"), AudioManager.defaultCurve, loop: true, fadeDuration: 3, AudioManager.SearchForSong("Main").GetTrackTime());
                    break;
                case Area.blue:
                    ...
                    break;
                case Area.yellow:
                    AudioManager.CrossFade(AudioManager.SearchForLayer("Fast"), AudioManager.SearchForLayer("Slow"), AudioManager.defaultCurve, AudioManager.defaultCurve, loop: true, fadeDuration: 3);
                    break;
                default:
                    break;
            }
